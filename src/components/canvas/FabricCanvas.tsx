...
export function FabricCanvas({
	...
	const saveCurrentPageData = useDesignStore(
		(state) => state.saveCurrentPageData
	);
	...
	const canvasEvents = useMemo(
		() => ({
			...
			"object:scaling": (e: any) => {
				...
				// 对于文本对象，缩放改为改变宽度（实现折行而不是调整字号）
				if (target && target.type === "textbox") {
					const scaleX = target.scaleX || 1;
					// 只有当比例变化明显时才调整，避免浮点数抖动
					if (scaleX !== 1) {
						// 计算新宽度：当前宽度 * X轴缩放比例
	...
	return (
		...
		>
			...
			<div className="absolute bottom-4 right-4" ref={exportMenuRef}>
				...
				{isExportMenuOpen && (
					<div
						className="absolute bottom-full right-0 mb-2 w-56 bg-white rounded-lg
